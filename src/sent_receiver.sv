...
module sent_receiver (
    ...
    input logic       crc_check_enable,  // reject frames with a bad CRC, latched once per frame
  ...
  logic crc_enable_d, crc_enable_q;  // latched once per frame
  ...
  always_comb begin
    ...
    case (state_q)
      ...
      ST_NIBBLE: begin
        if (sent_timeout) begin
          ...
          if (tick_cnt_q < 16'd12 || tick_cnt_q > 16'd27) begin
            ...
            if (nibble_id_q == nibble_count_q + 3'd1) begin
      ...
      ST_CRC: begin
        ...
        if (!crc_enable_q || frame_shift_q[3:0] == crc_q) begin
          frame_valid_d = 1'b1;
